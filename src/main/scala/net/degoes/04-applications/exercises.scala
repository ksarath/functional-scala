...
object exercises extends App {
  ...
  // Queue version
  def crawlIOPar2[E: Monoid, A: Monoid](
                                        seeds     : Set[URL],
                                        router    : URL => Set[URL],
  ...
  ): IO[Nothing, (Fiber[Nothing, Unit], Ref[(Crawl[E, A], Set[URL])])]  = {
  ...
  //
  trait Console[F[_]] {
    // set of operations for this effect
  ...
  def chooseWord[F[_]: Random: Functor]: F[String] =
    nextInt[F](Dictionary.length).map(index =>
      Dictionary.lift(index).getOrElse("")
    )
  ...
  def testGame(data: TestData): IO[Nothing, TestData] =
    for {
      ref <- Ref(data)
      _ <- ({
        implicit val F = createTestInstance(ref)
  ...
  def getURL(url: URL): IO[Exception, String] =
    for {
      ...
      _ <- (for {
        exitResult <- IO.async[Nothing, ExitResult[Exception, String]](
          k =>
            blockingPool.submit(new Runnable() {
              def run: Unit =
                try {
                  k(
                    ExitResult.Completed(
                      ExitResult
                        .Completed(scala.io.Source.fromURL(url.url)(scala.io.Codec.UTF8).mkString)
                    )
                  )
                } catch {
  ...
  object test {
    ...
    val getURL: URL => IO[Exception, String] =
      (url: URL) =>
        SiteIndex
          .get(url)
          .fold[IO[Exception, String]](IO.fail(new Exception("Could not connect to: " + url)))(
            IO.now(_)
        )
